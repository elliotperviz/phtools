...
subroutine qextract
  ...
  do i = npG+1, npG+npH
    if ( warnH ) then
      qHcheck(:) = 2.d0 * q(i,:)
      if ( (npG == 1) .and. (.not. ( (abs(qHcheck(1)-floor(qHcheck(1)))<tiny(1.d0)) .and. (abs(qHcheck(2)-floor(qHcheck(2)))<tiny(1.d0)) &
              .and. (abs(qHcheck(3)-floor(qHcheck(3)))<tiny(1.d0)) ) ) ) then
        write(*,'(a)') '  WARNING: H set present but not following Gamma, the output cannot be used for phind.'
        write(*,'(a)') '           Use q4phi to obtain an ordered set.'
        warnH = .false.
      else if ( (npG == 0) .and. (.not. ( (abs(qHcheck(1)-floor(qHcheck(1)))<tiny(1.d0)) .and. (abs(qHcheck(2)-floor(qHcheck(2)))<tiny(1.d0)) &
              .and. (abs(qHcheck(3)-floor(qHcheck(3)))<tiny(1.d0)) ) ) ) then
